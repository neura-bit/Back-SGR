...
public class MensajeroMetricsService {
    ...
    public MensajeroMetricsDTO getMetricasMensajero(Long idMensajero, LocalDate fechaInicio, LocalDate fechaFin) {
        ...
        Usuario mensajero = usuarioRepository.findById(idMensajero)
                .orElseThrow(() -> new IllegalArgumentException("Mensajero no encontrado con id: " + idMensajero));
        ...
        int tareasCompletadas = (int) tareas.stream()
                .filter(t -> t.getEstadoTarea() != null && "COMPLETADA".equals(t.getEstadoTarea().getNombre()))
                .count();
        int tareasPendientes = (int) tareas.stream()
                .filter(t -> t.getEstadoTarea() != null && "PENDIENTE".equals(t.getEstadoTarea().getNombre()))
                .count();
        int tareasEnProceso = (int) tareas.stream()
                .filter(t -> t.getEstadoTarea() != null && "EN PROCESO".equals(t.getEstadoTarea().getNombre()))
                .count();
    ...
    public List<MensajeroMetricsDTO> getComparativoMensajeros(LocalDate fechaInicio, LocalDate fechaFin) {
        ...
        for (Usuario mensajero : mensajeros) {
            try {
                MensajeroMetricsDTO dto = getMetricasMensajero(mensajero.getIdUsuario(), fechaInicio, fechaFin);
                metricas.add(dto);
            } catch (Exception e) {
                // Si hay error con un mensajero específico, continuar con los demás
                System.err.println(
                        "Error obteniendo métricas para mensajero " + mensajero.getIdUsuario() + ": " + e.getMessage());
            }
        }
